...
// ── API via servidor /api/sheets ──────────────────────────────────────────────
async function sheetCall(action, range, values) {
...
const getRange   = (r)    => sheetCall("get",    r);
const putRange   = (r, v) => sheetCall("put",    r, v);
const appendRange= (r, v) => sheetCall("append", r, v);
const clearRange = (r)    => sheetCall("clear",  r);
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const hP = await getRange(`${TABS.P}!A1`);
        if(!hP.values){
          // Primera vez: inicializar hojas
          await appendRange(`${TABS.P}!A1`,[["id","nombre","prov","stock","min","max"]]);
          await appendRange(`${TABS.P}!A1`,INIT_PRODS.map(p=>[p.id,p.nombre,p.prov,p.stock,p.min,p.max]));
          await appendRange(`${TABS.PR}!A1`,[["id","nombre"]]);
          await appendRange(`${TABS.PR}!A1`,INIT_PROVS.map(p=>[p.id,p.nombre]));
          await appendRange(`${TABS.M}!A1`,[["fecha","hora","producto","proveedor","tipo","cantidad","stock_anterior","stock_nuevo","usuario"]]);
          setProds(INIT_PRODS); setProvs(INIT_PROVS);
        } else {
          const [rP,rPr,rM] = await Promise.all([
            getRange(`${TABS.P}!A2:F1000`),
            getRange(`${TABS.PR}!A2:B1000`),
            getRange(`${TABS.M}!A2:I1000`),
          ]);
          setProds(rP.values?.length ? rP.values.map(r=>({id:r[0],nombre:r[1],prov:r[2],stock:+r[3],min:+r[4],max:+r[5]})) : INIT_PRODS);
          setProvs(rPr.values?.length ? rPr.values.map(r=>({id:r[0],nombre:r[1]})) : INIT_PROVS);
  ...
  async function saveProds(p){
    setSaving(true);
    try{
      await clearRange(`${TABS.P}!A2:F1000`);
      if(p.length) await putRange(`${TABS.P}!A2`,p.map(x=>[x.id,x.nombre,x.prov,x.stock,x.min,x.max]));
    }finally{setSaving(false);}
  }
  async function saveProvs(p){
    await clearRange(`${TABS.PR}!A2:B1000`);
    if(p.length) await putRange(`${TABS.PR}!A2`,p.map(x=>[x.id,x.nombre]));
  }
  async function logMov(prod,tipo,cant,sa,sn){
    ...
    await appendRange(`${TABS.M}!A1`,[[fecha,hora,prod.nombre,pnom,tipo,cant,sa,sn,user]]);
  ...
  async function updateStock(id,nv){
    ...
    const tipo=nv>prod.stock?"Entrada":"Salida";
    const diff=Math.abs(nv-prod.stock);
  ...
  return(
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#F0F2F7",minHeight:"100vh",maxWidth:900,margin:"0 auto"}}>
      ...
      <div style={{padding:16}}>
        ...
        {tab==="movimientos"&&(
          <div>
            <div style={{marginBottom:12,fontSize:13,color:"#667085"}}>{movs.length} movimientos · Guardado en Google Sheets</div>
            <div style={{background:"#fff",borderRadius:12,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,0.07)"}}>
              {movs.length===0
                ?<div style={{padding:32,textAlign:"center",color:"#aaa"}}>Sin movimientos aún.</div>
